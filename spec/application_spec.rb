...
describe Application do
  ...
  it 'starts and ends a shift' do
    ...
    sleep 1
    expect(@projection.shift_by_id('1234')).to eq(start_time: start_time, employee_id: 4, aggregate_id: '1234')

    end_time = Time.now.iso8601
    @command_handler.handle(Commands::EndShift.new(aggregate_id: '1234', end_time: end_time))
    sleep 1
    expect(@projection.shift_by_id('1234')).to eq(start_time: start_time, employee_id: 4, end_time: end_time, aggregate_id: '1234')
  end
  ...
  it 'starts multiple shifts' do
    ...
    expect(@projection.shifts).to eq([
      { start_time: start_time, employee_id: 4, aggregate_id: '1234' },
      { start_time: start_time, employee_id: 5, aggregate_id: '5678' },
    ])
  end
  ...
  it "errors when ending a shift that hasn't started" do
    ...
    end.to raise_error(Shift::ShiftNotStarted)
  end
end
